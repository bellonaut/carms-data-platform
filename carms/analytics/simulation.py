...
def _load_supply(session: Session) -> Dict[Tuple[str, str], int]:
    rows = session.exec(select(SilverProgram.province, SilverProgram.discipline_name, SilverProgram.quota)).all()
...
def _apply_quota_shock(
    supply: Dict[Tuple[str, str], int],
    multiplier: float,
    target_provinces: Optional[List[str]],
    target_disciplines: Optional[List[str]],
...
def _dirichlet_weights(base_weights: Dict[Tuple[str, str], float], rng: np.random.Generator) -> Dict[Tuple[str, str], float]:
    keys = list(base_weights.keys())
    base = np.array([base_weights[k] for k in keys], dtype=float)
    alpha = base / base.sum() * DIRICHLET_CONC
    draw = rng.dirichlet(alpha)
    return {k: float(v) for k, v in zip(keys, draw)}
...
def run_simulation(session: Session, params: SimulationParams) -> Tuple[UUID, List[GoldMatchScenario]]:
    ...
    if params.scenario_type == "quota_shock":
        supply = _apply_quota_shock(base_supply, params.quota_multiplier, params.target_provinces, params.target_disciplines)
    if params.scenario_type == "preference_shift":
        weights = _apply_preference_shift(weights, params.shift_pct, params.target_provinces, params.target_disciplines)
    ...
    for _ in range(params.iterations):
        ...
        for key, demand in zip(keys, demand_draw):
